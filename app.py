...
def make_label_50x30(sku, title, spec, remark):
    """
    生成 LxU 专属 50x30mm 高清标签 (备注增强版)
    - 备注格式：内容两侧自动添加横杠 (例如: -绿色-)
    - 布局：底行同行显示，备注居右对齐
    """
        ...
        for i in range(1, n - 1):
            for j in range(i + 1, n):
                ...
                if get_w(l1) <= max_text_width and get_w(l2) <= max_text_width and get_w(l3) <= max_text_width:
                    w_diff = max(get_get_w := get_w(l1), get_w(l2), get_w(l3)) - min(get_w(l1), get_w(l2), get_w(l3))
...
st.success("✅ **细节微调**：仓库备注现已自动添加横杠装饰 (例如：-绿色-)。")
...
with col1:
    ...
    v_remark = st.text_input("仓库备注 (内容会自动加杠)", "绿色") 
...
with col2:
    ...
    if 'l_img' in st.session_state:
        st.image(st.session_state.l_img, caption="1000x600 px (300 DPI 细节优化版)", use_column_width=True)
        # 下载
