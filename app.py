...
def make_label_50x30(sku, title, spec):
    """
    生成 LxU 专属 50x30mm 高清标签 (视觉平衡版)
    - 扫码区：数字位置 270，与条形码紧凑不重叠
    - 字体权重：规格(最厚4层) > 品名/入库码(中等2层) > 底部(常规1层)
    """
...
st.info("💡 **修正完成**：已移除多余插件解决报错。支持 PNG/PDF 双格式下载。")
